...
public class ExportedInbox {
  ...
  // Whether email authentication checks are enabled
  // Null defaults to true for backward compatibility
  ...
  /**
   * Returns whether email authentication checks are enabled.
   *
   * @return {@code true} if enabled, {@code false} if disabled, {@code null} if not specified
   */
   ...
   * <p>Defaults to {@code true} for backward compatibility.
   *
   * @return {@code true} if enabled or not specified, {@code false} if explicitly disabled
   */
